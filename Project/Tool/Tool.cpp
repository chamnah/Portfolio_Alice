...
BOOL CToolApp::InitInstance()
{
	...
	//CleanState(); // 이걸로 Dock Wnd를 초기화한 상태로 불러올수 있다.
	...
	pFrame->LoadFrame(IDR_MAINFRAME,
		...
		nullptr);
	RECT rt = {};
	GetClientRect(pFrame->GetGameHwnd(), &rt);
	CCore::GetInst()->Init(tResolution(rt.right - rt.left, rt.bottom - rt.top), pFrame->GetGameHwnd(),true);

	CTestMgr::GetInst()->Init();
...
BOOL CToolApp::OnIdle(LONG lCount)
{
	...
	CTestMgr::GetInst()->Update();
	CCore::GetInst()->Render();
	CTestMgr::GetInst()->Render();
